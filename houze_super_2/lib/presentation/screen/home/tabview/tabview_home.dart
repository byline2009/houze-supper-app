...
class _TabViewHomeState extends State<TabViewHome> {
          ...
          if (overlayState is PickBuildingSuccessful &&
            ...
            return CustomRefreshIndicator(
              ...
              child: CustomScrollView(
                ...
                slivers: [
                  /* Building */
                  if (overlayState.currentBuilding.id?.isNotEmpty == true)
                  ...
                  /* Tien ich toa nha*/
                  if (moduleShow["facilities"] == true) WidgetFacilitiesBox()
                ],
              ),
            );
          }
  ...
  void onDismissFirstBrandPopup(BuildContext context) {}
    ...
    await Future.delayed(Duration.zero, () async {
      ...
      if (version.forceUpdate == true) {
        if (Platform.isAndroid) {
          if (compareVerion(version.citizenAndroid!, packageInfo.version))
            _isValid = true;
        } else if (Platform.isIOS) {
          if (compareVerion(version.citizenIos!, packageInfo.version))
            _isValid = true;
        }
      }
      if (_isValid) {
  ...
  bool compareVerion(String v1, String v2) {
    ...
    for (var i = 0; i < a1.length; i++) {
      sum1 += (int.parse(a1[i]) * pow(100, (a1.length - 1 - i)).toInt());
      sum2 += (int.parse(a2[i]) * pow(100, (a1.length - 1 - i)).toInt());
    }
    return (sum1 > sum2);
  }

  void processAfterGetBuildingSuccess(BuildingMessageModel model) async {
    final bool _valid = _checkBuildingShownFirstTime(model.id!);
    if (_valid) {
      await Storage.saveWelcome(model.id!).then((saveSusscess) async {
        if (saveSusscess) {
  ...
  void _navigateToNoBuildingPage() {
    AppRouter.pushAndRemoveUntil(
      Storage.scaffoldKey.currentContext!,
      AppRouter.PAGE_404_NO_BUILDING,
    );
  }
}
