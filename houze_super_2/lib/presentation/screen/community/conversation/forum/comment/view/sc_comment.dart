...
class SocialCommentScreenArgument {
  ...
  const SocialCommentScreenArgument(
      ...
      this.callback,
      required this.renderedInDiscussion});
}

class SocialCommentScreen extends StatefulWidget {
  final SocialCommentScreenArgument args;

  const SocialCommentScreen({Key? key, required this.args}) : super(key: key);

  @override
  SocialCommentScreenState createState() => SocialCommentScreenState();
}

class SocialCommentScreenState extends RouteAwareState<SocialCommentScreen> {
  late SocialCommentScreenArgument args;
  ...
  void initState() {
    ...
    //Firebase Analytics
    if (args.renderedInDiscussion) {
      GetIt.instance<FBAnalytics>().sendEventViewAllCommentsDiscussion(
          userID: Storage.getUserID() ?? "");
    } else {
      GetIt.instance<FBAnalytics>()
          .sendEventViewAllCommentsVoting(userID: Storage.getUserID() ?? "");
    }
  }
                        ...
                        child: TextButton(
                          style: ButtonStyle(
                            ...
                            textStyle: MaterialStateProperty.resolveWith(
                              (states) => TextStyle(color: Color(0xff808080)),
                            ),
                          ...
                          child: Text(
                            ...
                            // ),
                            style: AppFont.MEDIUM_GRAY_808080_12,
                          ),
                        ),
                      ),
                    )
                              ...
                              WidgetInputCommentBottom(
                                ...
                                callback: !this._didTap
                                    ? () async {
                                        ...
                                        if (args.renderedInDiscussion) {
                                          GetIt.instance<FBAnalytics>()
                                              .sendEventCommentDiscussion(
                                                  userID: Storage.getUserID() ??
                                                      "");
                                        } else {
                                          GetIt.instance<FBAnalytics>()
                                              .sendEventCommentVoting(
                                                  userID: Storage.getUserID() ??
                                                      "");
                                        }
                                        try {
        ...
        child: BlocBuilder<PollCommentBloc, PollCommentState>(
          builder: (_, PollCommentState state) {
            ...
            if (state is GetPollCommentListSuccessful && _listTemp.isEmpty) {
              ...
              totalCommentController.sink.add(state.totalCount ?? 0);
            }
                ...
                child: SmartRefresher(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return WidgetPollCommentItem(
                        comment: _list[index],
                        reportDoneCallback: (comment) {
                          _onRefresh();
                        },
                      );
                    },
