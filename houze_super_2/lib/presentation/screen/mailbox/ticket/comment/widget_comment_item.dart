...
class _WidgetCommentItemState extends State<WidgetCommentItem> {
  ...
  Widget _imageView(
    ...
    try {
      ...
      return Padding(
        ...
        child:
            Text(LocalizationsUtil.of(context).translate("feedback_msg_error"),
  ...
  Widget build(BuildContext context) {
    ...
    String _role() {
      var createdBy = widget.comment.createdBy;
      if (createdBy?.role == "" && createdBy?.isStaff == false) {
        return LocalizationsUtil.of(context).translate('is_admin');
      }
      if (createdBy?.role == "staff" && createdBy?.isStaff == true) {
        return LocalizationsUtil.of(context).translate('support_staff');
      }
      return "";
    }
                  ...
                  : Center(
                      child: Text(
                        widget.comment.createdBy?.fullname?[0].toUpperCase() ??
                            "",
                        style:
                            AppFont.BOLD_BLACK_20.copyWith(color: Colors.white),
                      ),
                    ),
            ),
          ),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            getLanguage.name != 'Tiếng Việt'
                                ? Container(
                                    child: widget.comment.description != null &&
                                            widget
