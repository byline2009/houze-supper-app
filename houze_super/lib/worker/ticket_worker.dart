...
import 'package:houze_super/presentation/screen/community/conversation/chat/models/chat_image_model.dart';
...
class TicketRequest extends OauthAPI {
  ...
  Future<dynamic> createPollCommentImage(String url, String token, File image,
      {bool isMicro: false}) async {
    try {
      FormData formData = FormData.fromMap({
...
Future<dynamic> uploadPollCommentImageWorker(ArgUpload arg) async {
  //Update ticket image
  final ticketRequest = TicketRequest(
    arg.oauthUrl,
    arg.url,
  ...
  final result = await ticketRequest.createPollCommentImage(
    arg.url,
    arg.token,
    arg.file,
    isMicro: arg.isMicro,
