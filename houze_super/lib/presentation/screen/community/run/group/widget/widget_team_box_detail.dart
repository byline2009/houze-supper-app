...
class TeamDetailBoxWidget extends StatelessWidget {
          ...
          child: Column(
            ...
            children: [
              ...
              DistanceAndTimeStatistic(
                distance: group.distance.toDouble(),
                accumulationTime: group.totalTime.toDouble(),
              ),
              const SizedBox(height: 20),
              linearProgressBar(),
              Row(
                      ...
                      child: RichText(
                        text: TextSpan(
                          ...
                          children: <TextSpan>[
                            ...
                            TextSpan(text: event.getDaysLeft(context))
                          ],
                        ),
                      ),
                    ),
  ...
  Widget chatBubble(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        ...
        if (group.joined != null && group.joined.length > 0) {
          users = group.joined
              .map(
                (e) => UserModel(
                  userId: e.user.id,
  ...
  Widget linearProgressBar() {
    double value = group.distanceKm / event.targetRun.toDouble();
