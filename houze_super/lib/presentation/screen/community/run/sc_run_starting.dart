...
import 'package:houze_super/presentation/common_widgets/stateless/index.dart';
...
class RunStartingScreen extends StatefulWidget {
  ...
  const RunStartingScreen({this.args});

  @override
  _RunStartingScreenState createState() => _RunStartingScreenState();
}

class _RunStartingScreenState extends State<RunStartingScreen> {
  Timer timer;
  ...
  double totalDistance, distance = 0.0;
 ...
 */
  ...
  Future<void> _onStop() async {
    ...
    points.addAll(locations.map((e) => LatLng(e.latitude, e.longitude)));
  ...
  Widget build(BuildContext context) {
    final String timeStr = formatTime(time) ?? '00:00:00';
    ...
    //             Navigator.of(context).popUntil(
    //               ModalRoute.withName(AppRouter.COMMUNITY_RUN_MAIN_PAGE),
    //             );
    //           },
    //         ),
    //       ),
